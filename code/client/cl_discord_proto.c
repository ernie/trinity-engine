...
int Discord_BuildSetActivity( char *out, int outSize, const discordActivity_t *act, int pid, int nonce ) {
	...
	char fields[ DISCORD_FIELD_SIZE * 6 ];
	int fp = 0, w, body;
...
void Discord_MapActivity( discordActivity_t *out, discordPhase_t phase,
	...
	switch ( phase ) {
		...
		case DISCORD_WATCHING_DEMO:
			...
			wantsMap = 1;
			break;
		case DISCORD_WATCHING_TVD:
			strcpy( out->details, "Watching a TrinityVision demo" );
			wantsMap = 1;
			break;
		case DISCORD_WATCHING_TV:
			strcpy( out->details, "Watching TrinityVision" );
			wantsMap = 1;
			break;
	}

	if ( wantsMap ) {
		/* prefer the worldspawn message (the map's display title, e.g. "The
		   Longest Yard"), stripped of color codes; fall back to the bsp name
		   when the message is unset or has no displayable text */
