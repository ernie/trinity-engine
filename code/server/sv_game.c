...
void SV_InitGameProgs( void ) {
	...
	gvm = VM_Create( VM_GAME, SV_GameSystemCalls, SV_DllSyscall, Cvar_VariableIntegerValue( "vm_game" ), qfalse );
	if ( !gvm ) {
...
qboolean SV_GameCommand( void ) {
	...
	// Intercept trinity_auth_ok to stash the canonical name on client_t
	// before the QVM sees the command. The QVM still processes the
	// command (sets trinityUserType, triggers announcement); the engine
	// just owns the name-lock field.
	if ( !Q_stricmp( cmd, "trinity_auth_ok" ) && Cmd_Argc() >= 4 ) {
		int slot = atoi( Cmd_Argv( 1 ) );
		if ( slot >= 0 && slot < sv_maxclients->integer ) {
			Q_strncpyz( svs.clients[slot].trinityName,
			            Cmd_Argv( 3 ),
			            sizeof( svs.clients[slot].trinityName ) );
			SV_UserinfoChanged( &svs.clients[slot], qtrue, qfalse );
		}
	}

	// Clear the name lock on auth failure so the client can /name freely.
	if ( !Q_stricmp( cmd, "trinity_auth_fail" ) && Cmd_Argc() >= 2 ) {
		int slot = atoi( Cmd_Argv( 1 ) );
		if ( slot >= 0 && slot < sv_maxclients->integer ) {
			svs.clients[slot].trinityName[0] = '\0';
		}
	}
